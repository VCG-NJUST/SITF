...
from normal import one_normal
import os
import numpy as np
import scipy.spatial as sp
from Pointfilter_Utils import pca_alignment
from addnoise import addgaussiannoise_1
from geomsample import geosample
...
class PointcloudPatchDataset(data.Dataset):
    ...
    def __init__(self, root=None,ori_root=None,root_noise=None, shapes_list_file=None, patch_radius=0.05, points_per_patch=500,
                 seed=None, train_state='train', shape_name=None,num=0):
        ...
        if self.train_state == 'evaluation' and shape_name is not None:
            ...
            origin_pts = np.load(os.path.join(self.ori_root, shape_name + '.npy'))
            origin_kdtree = sp.cKDTree(origin_pts)
            self.origin_shapes.append({'noise_pts': origin_pts, 'noise_kdtree': origin_kdtree})
    ...
    def __getitem__(self, index):
        ...
        origin_shape = self.origin_shapes[shape_ind]
        origin_patch_idx = origin_shape['noise_kdtree'].query_ball_point(noise_shape['noise_pts'][patch_ind],patch_radius)

        if len(origin_patch_idx) < 3:
            return None

        origin_patch_pts = origin_shape['noise_pts'][origin_patch_idx] - noise_shape['noise_pts'][patch_ind]
        origin_patch_pts /= patch_radius
        origin_patch_pts = np.array(np.linalg.inv(noise_patch_inv) * np.matrix(origin_patch_pts.T)).T

        origin_sample_idx = self.patch_sampling(origin_patch_pts)
        origin_patch_pts = origin_patch_pts[origin_sample_idx]


        if self.train_state == 'evaluation':

            return torch.from_numpy(noise_patch_pts), torch.from_numpy(noise_patch_inv), \
                   noise_shape['noise_pts'][patch_ind],torch.from_numpy(origin_patch_pts)
        ...
        #origin_shape=self.origin_shapes[shape_ind//6]
        #origin_patch_idx=origin_shape['noise_kdtree'].query_ball_point(noise_shape['noise_pts'][patch_ind], patch_radius)
        ...
        return torch.from_numpy(noise_patch_pts), torch.from_numpy(gt_patch_pts), \
               torch.from_numpy(gt_patch_normal), torch.from_numpy(support_radius),torch.from_numpy(origin_patch_pts)
