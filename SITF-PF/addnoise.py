...
def addgaussiannoise():
    ...
    with open(os.path.join('D:/jk/self-supervision/train', 'gs_noise_1.txt'), 'r') as f:
    ...
    for shape_id,shape_name in enumerate(shape_names):
        ...
        for mean in means:
            ...
            out=pts+noise
            #print(out.dtype)
            np.save(os.path.join('D:/jk/self-supervision/train',shape_name+'_'+str(mean)+'.npy'),out.astype(np.float32))
            #np.save(os.path.join('D:/jk/self-supervision/origin', shape_name + '_' + str(mean) + '.npy'),out.astype(np.float32))
...
def addpulsenoise():
    noise_levels =[0.0025,0.005,0.01,0.015,0.025]
    with open(os.path.join('D:/jk/self-supervision/train', 'pulse_noise.txt'), 'r') as f:
        shape_names = f.readlines()
    shape_names = [x.strip() for x in shape_names]
    shape_names = list(filter(None, shape_names))
    for shape_id, shape_name in enumerate(shape_names):
        pts = np.load(os.path.join('D:/jk/self-supervision/train', shape_name + '.npy'))
        bbdiag = float(np.linalg.norm(pts.max(0) - pts.min(0), 2))
        ...
        for noise_level in noise_levels:
            ...
            pulse_out = pts + f
            #np.save(os.path.join('D:/jk/self-supervision/origin', shape_name + '_' + str(noise_level) + '.npy'),
            #        pulse_out.astype(np.float32))
            np.save(os.path.join('D:/jk/self-supervision/train', shape_name +'_'+ str(noise_level) + '.npy'), pulse_out.astype(np.float32))


def adduniformnoise():
    noise_levels = [0.0025, 0.005, 0.01, 0.015, 0.025]
    with open(os.path.join('D:/jk/self-supervision/train', 'uniform_noise.txt'), 'r') as f:
        shape_names = f.readlines()
    shape_names = [x.strip() for x in shape_names]
    shape_names = list(filter(None, shape_names))
    for shape_id, shape_name in enumerate(shape_names):
        ...
        for noise_level in noise_levels:
            ...
            average_out = pts + s_noise
            np.save(os.path.join('D:/jk/self-supervision/train', shape_name + '_' + str(noise_level) + '.npy'),
                    average_out.astype(np.float32))
            #np.save(os.path.join('D:/jk/self-supervision/origin', shape_name + '_' + str(noise_level) + '.npy'),
                    #average_out.astype(np.float32))
def addmixnoise():
    means = [0.0025, 0.005, 0.01, 0.015, 0.025]
