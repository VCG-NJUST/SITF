...
def eval(opt):
    ...
    with open(os.path.join(opt.testset, '1.txt'), 'r') as f:
    ...
    for shape_id, shape_name in enumerate(shape_names):
        ...
        for eval_index in range(2):
            ...
            pointfilter_eval = pointfilternet(num=eval_index).cuda()
...
if __name__ == '__main__':
    ...
    parameters.testset = 'D:/jk/self-supervision/testraw'      #测试模型路径
    #parameters.eval_dir = './Summary/pre_train_model/'
    parameters.batchSize = 256
    parameters.workers = 4
    parameters.save_dir = 'D:/jk/self-supervision/result/'      #去噪后模型存放路径
    parameters.eval_dir = 'D:/jk/self-supervision/Summary/transformer'   #神经网络参数路径，不用动
