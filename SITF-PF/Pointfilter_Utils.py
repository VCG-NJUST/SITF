...
def parse_arguments():
    ...
    parser.add_argument('--testset', type=str, default='D:/jk/self-supervision/test', help='testing set file name')
    parser.add_argument('--denoiseset', type=str, default='D:/jk/self-supervision/noise', help='denoising set file name')
    parser.add_argument('--save_dir', type=str, default='D:/jk/self-supervision/result', help='')
    parser.add_argument('--denoise_dir', type=str, default='D:/jk/self-supervision/denoised', help='')
    parser.add_argument('--origin', type=str, default='D:/jk/self-supervision/origin', help='')
    ...
    parser.add_argument('--eval_dir', type=str, default='D:/jk/self-supervision/Summary/Train', help='')
...
class PID():
    ...
    def compute_pid(self,error):
        ...
        #sigma_error=error[0]+ error[1]+error[2]
        self.DTerm=0
        sigma_error=sum(error)
        self.PTerm = self.Kp * error[-1]   #比例
        self.ITerm = (self.Ki/3)* sigma_error  #积分
       # self.DTerm = self.Kd * delta_error
        #output = self.PTerm + self.ITerm + self.DTerm
        return self.ITerm

def feature_enhance(noise,pid_result):
    #error0 = np.squeeze(error[0].data.cpu().numpy())
    #error1 = np.squeeze(error[1].data.cpu().numpy())
    noise = np.squeeze(noise.data.cpu().numpy())
    #pid_noise_patch = np.squeeze(pid_noise_patch.data.cpu().numpy())
    pid_result = np.squeeze(pid_result.data.numpy())
    #displace = np.zeros(shape=error[0].shape)
    # index = np.random.choice(500, size=20, replace=False)
    pid_noise_patch=noise-pid_result

    return torch.from_numpy(pid_noise_patch)

def error_compute(noise,latent):
    latent = np.squeeze(latent.data.cpu().numpy())
    noise = np.squeeze(noise.data.cpu().numpy())
    #init_error = np.squeeze(init_error.data.numpy())
    # displace = np.zeros(shape=error[0].shape)
    # index = np.random.choice(500, size=20, replace=False)
    init_error = noise - latent

    return torch.from_numpy(init_error)