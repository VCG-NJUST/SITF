    ...
    with torch.no_grad():
        for shape_id, shape_name in enumerate(shape_names):
            ...
            for iter_index in range(2):
                ...
                pointfilter_eval = pointfilternet(num=iter_index).cuda()
                model_filename = os.path.join(opt.eval_dir, 'model_full_ae_'+str(i)+'_'+str(iter_index)+'.pth')
                ...
                for batch_ind, data_tuple in enumerate(denoise_dataloader):
                    ...
                    noise_patch, noise_inv, noise_disp,origin_noise = data_tuple
                    ...
                    predict = pointfilter_eval(noise_patch,origin_noise)
...
def denoise1(opt,i,num):
    ...
    with open(os.path.join(opt.trainset, 'denoise_gs.txt'), 'r') as f:
        shape_names = f.readlines()
    shape_names = [x.strip() for x in shape_names]
    shape_names = list(filter(None, shape_names))
    ...
    with torch.no_grad():
        for shape_id, shape_name in enumerate(shape_names):
            ...
            pointfilter_eval = pointfilternet(num=num).cuda()
...
if __name__ == '__main__':
    ...
    parameters.denoiseset = 'D:/jk/self-supervision/noise'
    ...
    parameters.denoise_dir= 'D:/jk/self-supervision/denoised'
    #parameters.eval_dir='D:/jk/self-supervision/Summary/transformer'
    parameters.eval_iter_nums = 2
    parameters.patch_radius = 0.05
    denoise(parameters,0)
